...
Rollo::Rollo(const Rectangle& rectangle, const ldtk::ArrayField<ldtk::IntPoint> path_points) :
	Enemy({ rectangle.x,rectangle.y,12,26 }, ROLLO, b2_dynamicBody),
	m_path_points(path_points)
{
	...
	//feet collider box
	m_feet_sensor = util::SimpleSensor(m_body, "rol_feet", 0.3f, 0.3f, 0, 0.80f);

	//player agro sensor
	m_agro_sensor = util::SimpleSensor(m_body, "rol_agro", 2.0f, 2.0f, -3.0, 0.0);
	m_agro_sensor = util::SimpleSensor(m_body, "rol_agro", -2.0f, 2.0f, 3.0, 0.0);
	...
	//attack sensor
	m_attack_sensor = util::SimpleSensor(m_body, "rol_att", 1.0f, 0.8f, 0.0, 0.0);

	//left&right collider boxes
	m_left_sensor = util::SimpleSensor(m_body, "rol_l_s", 0.05f, 0.80f, -0.6f, 0);
	m_right_sensor = util::SimpleSensor(m_body, "rol_r_s", 0.05f, 0.80f, 0.6f, 0);
...
void Rollo::Die(int death_option)
{

	switch (death_option)
	{
	case 1:
		SetAnimation("ROL_DEAD");
...
void Rollo::InitAnimations()
{
	animations->InitializeRolloAnimations();
	SetAnimation("ROL_IDLE");
}

void Rollo::UpdateIdleState(float dt)
{
...
void Rollo::UpdateRunningState(float dt)
{
	if (!player_in_agro)
	{
		state = EnemyState::Idle;
...
void Rollo::SetAttacking()
{
	if (GetRandomValue(0, 1) == 0)
	{
		SetAnimation("ROL_ATT1");
		PlaySound(SoundManager::sounds["slime1"]);
		state = EnemyState::Attacking;
	}
	else
	{
		SetAnimation("ROL_ROLL");
		PlaySound(SoundManager::sounds["slime1"]);
		state = EnemyState::Attacking;
	}
...
void Rollo::UpdateAttackingState(float dt)
{
	if (animations->m_CurrentActiveAnimation == "ROL_ATT1")
	{
	...
	else if (animations->m_CurrentActiveAnimation == "ROL_ROLL")
	{
...
void Rollo::UpdateHurtingState(float dt)
{

}

void Rollo::UpdateDyingState(float dt)
{
...
void Rollo::UpdatePatrollingState(float dt)
{
